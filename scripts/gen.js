...
function createGenesisBlock(options) {
  ...
  if (!flags) {
    flags = new Buffer(
      'The Times 10/May/2017 Sacking FBI chief had nothing to do with Russia inquiry, says Trump',
      'ascii');
  }
...
if (mineGenesis) {
  util.log('generating new genesis block');
  var nonce = 0;
  var hashGenesisBlock = 0x001;
  var powLimit = new BN(
  ...
  while (main.hash > powLimit) {
    ...
    // util.log('main hash: %s', main.rhash(), ' nonce: ', nonce);
    nonce++;
  }
} else {
  main = createGenesisBlock({
